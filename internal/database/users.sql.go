...
type UpgradeUserByIdParams struct {
	IsChirpyRed bool
	ID          uuid.UUID
}

func (q *Queries) UpgradeUserById(ctx context.Context, arg UpgradeUserByIdParams) (User, error) {
	row := q.db.QueryRowContext(ctx, upgradeUserById, arg.IsChirpyRed, arg.ID)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
